...
description: "useTextDirection is a React hook that reads and updates the dir attribute on an element — control ltr and rtl text direction for right-to-left layouts."
---
# useTextDirection
...
- **The element wins**: `initialValue` is a fallback, not an override. If the target element already carries a `dir` attribute, the hook reports that value — on the very first render, not after an effect — so an RTL document never flashes `ltr`. `initialValue` is used only when the element has no `dir`.
- **SSR-safe**: No `document` access occurs on the server; server rendering returns `initialValue` (default: `"ltr"`) and a no-op setter. Because the client reads the element on its first render, pass an `initialValue` matching the direction of your server-rendered markup, or the two will disagree and React will report a hydration mismatch.
