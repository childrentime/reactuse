...
- fix(useKeyModifier): detach the listeners and follow a changed modifier. The four `document` listeners were registered through `useMount`, which discards the callback's return value, so their cleanup never ran — and even called directly it passed freshly built arrow functions to `removeEventListener`, which matches by reference. Every mount left four more listeners on `document`, each calling `setState` on an unmounted component whenever a key or button was pressed. `useMount`'s empty dependency list also froze the arguments, so going from `useKeyModifier('Shift')` to `useKeyModifier('Control')` kept reporting Shift, and the same held for `events`. Now registered in a `useDeepCompareEffect` keyed on `[modifier, events]` with one hoisted handler, the shape `useEventListener` already uses. Adds `useKeyModifier/index.spec.ts` — seven tests, three of which fail on 6.5.6. Thanks to @rawsun007 (#223)
- fix(useDoubleClick): keep the click count across renders and cancel the timer. `handle(...)` ran during render, so the `let count = 0` inside it was rebuilt on every render while `useEventListener` dispatched to the newest closure: a re-render between two clicks reset the count and both reported as single clicks. The `setTimeout` id was never stored either, so nothing could cancel it and the callback ran after unmount. Count and timer now live in a ref, one pair per event type so `click` and `touchend` stay independent; only the first click arms a timer, the teardown clears it, the count is reset before the callbacks run so a throwing callback cannot wedge the hook, and both callbacks go through `useLatest`. Spec grown from a placeholder to twelve tests, five of which fail on 6.5.6. Thanks to @rawsun007 (#224)
- fix(useMousePressed): attach the `touch` and `drag` listeners to a ref target. The target was resolved during render, where a ref handed to a child is still `null`, and the effect depended on that resolved element so it never re-ran to pick it up — with the usual `useRef(null)` pattern those listeners were never attached at all, while `mousedown` worked because it goes through `useEventListener`. The target now resolves inside the effect, keyed on `useStableTarget`. `onPressed` was also curried, so the add and the remove received different functions and `dragstart` and `touchstart` were never detached; there is one stable handler each now. Adds `useMousePressed/index.spec.ts` — six tests, two of which fail on 6.5.6. Thanks to @rawsun007 (#225)
...
- fix(useCssVar): stop rebuilding the MutationObserver and the returned setter on every render. `set` and `updateCssVar` were memoised on `[target, prop]`, and the effect that installs the observer depended on both plus `target`, so a getter target invalidated all of them every render. With `observe: true` the observer was disconnected and reconstructed each render — four observers across three renders, against one for a stable element — which both wastes work and leaves a gap where mutations go unseen; and the `set` handed back to callers changed identity every render, invalidating any consumer memo built on it. The target now resolves through `useStableTarget` and the callbacks read it from the ref, so `set` is stable for the hook's lifetime and the observer is built once per element. Spec grown from a stub to eleven tests
- fix(useScrollLock): key the lock effect on the element rather than on the getter. Follow-up to the 6.5.4 snapshot fix, which made the effect idempotent but left it re-running on every render whenever `target` was the `() => document.body` getter the docs recommend — re-writing `overflow: hidden` each time, and a trap for anything added to that effect later. `useStableTarget` ties it to the element it actually locks; the element-keyed snapshot stays as the invariant that guarantees one read per lock
- chore: audited every hook that accepts a `BasicTarget`. `useEventListener`, `useIntersectionObserver`, `useMutationObserver`, `useResizeObserver` and `useSticky` already resolved theirs through `useStableTarget`; the three above were the only ones depending on a raw target. The rest reach the DOM through `useEventListener`, `useResizeObserver` or `useEvent`, all of which are already stable

## 6.5.4(Aug 20, 2026)

- fix(useEventListener): pass the listener options back to `removeEventListener`. Registration called `addEventListener(name, fn, options)` but cleanup called `removeEventListener(name, fn)` with the options dropped, and the DOM matches a listener for removal on (type, callback, capture) — so anything registered with `{ capture: true }` (or the boolean `true` form) was never detached. A capture-phase listener kept firing after the component unmounted, survived a change of `eventName` or `options`, and accumulated one live listener per mount: a modal that intercepts clicks on `document` in the capture phase went on intercepting them for the rest of the session. `passive`, `once` and `{ capture: false }` were unaffected, since only `capture` participates in the removal match. Verified in jsdom and Chrome 151
- fix(useScrollLock): snapshot the original `overflow` once per lock, not on every render. The effect that applies the lock lists `target` in its dependencies, and the `() => document.body` getter the docs recommend for SSR safety is a fresh function on every render, so the effect re-ran constantly and re-executed `initialOverflowRef.current = element.style.overflow`. While locked that reads back the `hidden` the hook itself just wrote, so a single unrelated re-render with the modal open poisoned the saved value: `unlock()` then restored `overflow: hidden` and the page never scrolled again, an existing inline value such as `overlay` was swallowed, and the v6.5.3 unmount cleanup restored the poisoned value too. Only the happy path — lock, then unlock with no render in between — behaved correctly, which is why hand testing never caught it. The snapshot is now keyed on the locked element, so it happens once per lock however often the effect re-runs; if the target moves while locked, the element being released is restored before the new one is captured
