...
description: "useWindowFocus is a React hook that reactively tracks whether the browser window has focus, updating via the window focus and blur events."
---
# useWindowFocus

React Element Hook that tracks window focus with `window.onfocus` and `window.onblur` events

`useWindowFocus` reactively tracks whether the browser window currently has focus by listening to `window.onfocus` and `window.onblur` events. It returns a boolean that is `true` when the window is focused and `false` when it is not. You can optionally provide a default value for the initial state.
