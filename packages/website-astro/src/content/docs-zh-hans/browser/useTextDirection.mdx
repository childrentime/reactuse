...
description: "useTextDirection 是一个读取并更新元素 dir 属性的 React hook，用于控制 ltr 和 rtl 文本方向，适配从右到左的布局。"
---
# useTextDirection
...
- **以元素为准**：`initialValue` 是兜底值而非覆盖值。若目标元素已带有 `dir` 属性，hook 在首次渲染时就会报告该值（而不是等 effect 纠正），因此 RTL 文档不会先闪一下 `ltr`。只有元素没有 `dir` 时才会使用 `initialValue`。
- **SSR 安全**：服务端不会访问 `document`，服务端渲染返回 `initialValue`（默认 `"ltr"`）和空操作设置函数。由于客户端在首次渲染时就读取元素，请传入与服务端渲染标记方向一致的 `initialValue`，否则两边不一致，React 会报 hydration 不匹配。
