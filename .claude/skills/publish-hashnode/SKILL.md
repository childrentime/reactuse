...
opencli browser --session default fill <body-textarea-ref> "$MD" > /dev/null

opencli browser --session default eval '
...
```bash
opencli browser --session default open "https://hashnode.com/draft/<id>" > /dev/null
sleep 3
opencli browser --session default eval "document.querySelectorAll('textarea')[1].value.length"
```
...
```bash
opencli browser --session default eval '
const btns = Array.from(document.querySelectorAll("button")).filter(b => b.textContent.trim() === "Publish");
const dialogBtn = btns.find(b => b.closest("[role=dialog]"));
dialogBtn.click();
"clicked"
'
sleep 8
opencli browser --session default get url                                 # 应跳到 /edit/<cuid>
```

URL 进了 `/edit/<id>` 就是发出去了。如果还停在 `/draft/<id>`，多半是步骤 4 的 onChange 没跑、稿子是空的——回去走一遍验证流程。
...
```bash
opencli browser --session default open "https://reactuse.hashnode.dev/"
opencli browser --session default eval '
(() => {
  const articles = Array.from(document.querySelectorAll("article a, h2 a, h3 a"));
  return articles.slice(0, 3).map(a => ({text: a.textContent.trim().slice(0,80), href: a.href}));
})()
'
```
